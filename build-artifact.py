...
def datauri(p):
    kind = 'webp' if p.endswith('.webp') else 'png'
    return f'data:image/{kind};base64,' + base64.b64encode((root / p).read_bytes()).decode()
html = re.sub(r'src="(assets/[^"]+\.(?:png|webp))"', lambda m: f'src="{datauri(m.group(1))}"', html)
